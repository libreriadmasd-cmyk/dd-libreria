...
import { buildWhatsAppUrl, buildWhatsAppProductUrl } from "../../lib/whatsapp";
import { fetchProduct } from "../../lib/api";

export const WhatsAppFloating = () => {
  const { items, total, count } = useCart();
  const params = useParams();
  const skuFromRoute = params?.id;
  const [product, setProduct] = useState(null);

  useEffect(() => {
    if (!skuFromRoute) { setProduct(null); return; }
    let active = true;
    fetchProduct(skuFromRoute)
  ...
  const handleClick = () => {
    if (skuFromRoute && product) {
      window.open(buildWhatsAppProductUrl(product), "_blank", "noopener");
      return;
    }
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="fixed bottom-6 right-6 z-40 h-16 w-16 rounded-full bg-brand-teal hover:bg-brand-tealDark text-white shadow-xl shadow-teal-300/40 hover:shadow-teal-400/60 active:scale-95 transition-all inline-flex items-center justify-center group"
    ...
    >
      <span className="relative grid place-items-center">
        ...
        {!skuFromRoute && count > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[20px] h-[20px] px-1 rounded-full bg-brand-coral text-white text-[10px] font-bold grid place-items-center ring-2 ring-white">
            {count}
          </span>
        )}
      </span>
